...
const getSecret = async (secretName: string): Promise <StripeSecret> => {
    ...
    if (secretValue.SecretString === undefined) throw new Error('Secret is not found.');
...
export const handler = async (event: Payment, context: Context): Promise <Stripe.PaymentIntent> => {
    console.info({event});
